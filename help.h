...
bool isKeyPressed(int& whichKey)   //whichKey passed as reference.... 
{
	...
	key = GetAsyncKeyState(49);
	if (key == 1)
	{
		whichKey = 9;		// 1 if 1 key is pressed 
		return true;
	}
	key = GetAsyncKeyState(48);
	if (key == 1)
	{
		whichKey = 8;		// 1 if 0 key is pressed 
		return true;
	}

	key = GetAsyncKeyState(50);
	if (key == 1)
	{
		whichKey = 10;		// 1 if 2 key is pressed 
		return true;
	}
	key = GetAsyncKeyState(51);
	if (key == 1)
	{
		whichKey = 11;		// 1 if 3 key is pressed  uwu
		return true;
	}
